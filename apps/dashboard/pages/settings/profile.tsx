...
function OrganizationForm({}: Props) {
  ...
  const submitInvite = async (values: OrganizationInviteSchema) => {
    try {
      if (
        ...
        alert(
          'Has alcanzado el límite de miembros de tu plan. Por favor contacta a soporte@braviloai.com para actualizar tu plan.'
        );
        return;
      }
      ...
      if (
        ...
        toast.error(
          "Error al enviar invitación. Por favor contacta a soporte@braviloai.com si el problema persiste.",
          {
            duration: 5000,
          }
        );
  ...
  const handleDeleteMember = async (id: string) => {
    try {
      if (
        window.confirm(
          '¿Estás seguro de eliminar este miembro del equipo?'
        )
      ) {
        setState({
  ...
  return (
    <Stack gap={4} id="team">
      <SettingCard
        ...
        description="Personaliza el nombre y el icono de tu equipo"
        disableSubmitButton
      >
        <form
      ...
      <SettingCard
        ...
        description="Invita nuevos miembros a colaborar"
        disableSubmitButton
      >
